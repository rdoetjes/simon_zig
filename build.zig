...
pub const Simon = struct {
    simon: *microzig.EmbeddedExecutable,

    pub fn init(b: *Builder, optimize: std.builtin.OptimizeMode) Simon {
        var ret: Simon = undefined;
        inline for (@typeInfo(Simon).Struct.fields) |field| {
            const path = comptime root() ++ "simon/" ++ field.name ++ ".zig";
    ...
    pub fn install(examples: *Simon, b: *Builder) void {
        inline for (@typeInfo(Simon).Struct.fields) |field| {
